...
public class ArduinoSerProUsb implements SerialPortEventListener{
	...
	private static ArduinoSerProUsb instance = null;
	...
	public static ArduinoSerProUsb getInstance() throws IOException {
		if(instance==null){
			instance = new ArduinoSerProUsb();
		}
		return instance;
	}
	
	private ArduinoSerProUsb() throws IOException{
		try{
			initializeSerial();
	...
	public static void main(String[] args) {
		try {
			ArduinoSerProUsb comm = ArduinoSerProUsb.getInstance();
			Thread.sleep(1000);
			comm.commands.sendCommand0SumThreeNumbers(1, 2, 4);
			Thread.sleep(1000);
			comm.commands.sendCommand1SetLed13State(1);
			Thread.sleep(1000);
			comm.commands.sendCommand1SetLed13State(0);
			Thread.sleep(1000);
		} catch (IOException e) {
			e.printStackTrace();
